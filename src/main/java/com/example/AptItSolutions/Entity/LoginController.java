...
public class LoginController {
    ...
    public ResponseEntity<Login> registerUser(@RequestBody Login user) {
        try {
            Login savedUser = userService.saveUser(user);
    ...
    @PostMapping("/login")
    public ResponseEntity<Login> login(@RequestBody Login loginForm) {
        Login loginResponse = userService.login(loginForm.getEmail(), loginForm.getPassword());
